        ...
        while True:
            ...
            if done or t > t_max:
                if done:
                    ...
                    # _s_ = preprocess(s_)
                    # _s_ = np.concatenate((_s_, _s_, _s_, _s_), axis=3)
                    v_s_ = SESS.run(self.AC.v, {self.AC.s: _s})
                for r in buffer_r[::-1]:
                    v_s_ = r + GAMA*v_s_
...
while opt != 3:
    if opt == 2:
        ...
        np.save('A3C_v2_%.1fH' % c, ttmp) #保存一个小时内的数据
        ttmp = []
        c += 1.0
        REC = True
    worker_threads = []
    ...
    ttmp.append(max_p)
    print('this', len(GLOBAL_R),'round get point', max_p)
    # P_REC.append(ALL_p)
    ALL_p = []
    opt = time_count(long)
