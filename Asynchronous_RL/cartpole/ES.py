        ...
        if not COORD.should_stop():
            ...
            while True:
                ...
                if done:    #tell all the agent to learn this fail experience
                    ...
                    tmp.append(self.buffer_r[i])
                    R_B_q.append(tmp)
                    R_B_u.append(self.number)
                ...
                if done or ep_r > 199:   # update global and assign to local net
                    ...
                    if w:
                        ...
                        if len(self.share_experience) > 0:
                            # print('agent', self.number, 'learn success')
                            for exp in self.share_experience:
                                self.buffer_s.append(exp[0])
                                self.buffer_a.append(exp[1])
                                self.buffer_v_target.append(exp[2])
            ...
            if i[0] in np.array(self.buffer_s):
                ...
                if self.buffer_r[index] >= 0: # judge the success of the experience
                    for i in range(5):
                        ...
                        tmp.append(self.buffer_v_target[index])
                    SOLVED_EXP.append([R_B_u[place], tmp])
            place += 1

    def recieve(self):
        global SOLVED_EXP
        for item in SOLVED_EXP:
            # print(self.number, item, item[0])
            if item[0] == self.number:
                self.share_experience.append(item[1])
...
if __name__ == "__main__":
    ...
    for x in tqdm(range(MAX_GLOBAL_EP)):
        ...
        for worker in workers:
            job = lambda: worker.sharing()
            t = threading.Thread(target=job)
            t.start()
            share_threads.append(t)
        COORD.join(share_threads)
